...
type ClerkMode = "sign-in" | "sign-up";
...
export default function SignInFlow({ className, mode = "sign-in" }: SignInPageProps) {
  ...
  async function onGoogle() {
    ...
    try {
      ...
      setError(getClerkErrorMessage(e, "Google authentication failed."));
    ...
    try {
      ...
      setError(
        getClerkErrorMessage(
          e,
          mode === "sign-in" ? "Failed to sign in." : "Failed to sign up."
        )
      );
    } finally {
      setSubmitting(false);
  ...
  async function resendCode() {
    ...
    try {
      ...
      setError(getClerkErrorMessage(e, "Failed to resend code."));
    }
  }
  ...
  async function verifyCode(fullCode: string) {
    ...
    try {
      if (mode === "sign-up") {
        const res = await signUp!.attemptEmailAddressVerification({ code: fullCode });
        if (res.status === "complete") {
          await setActiveSignUp!({ session: res.createdSessionId });
          setStep("success");
          router.push("/");
      ...
      setError("This sign-in flow expects password login.");
      return false;
    } catch (e) {
      setError(getClerkErrorMessage(e, "Invalid code."));
      return false;
    } finally {
      setSubmitting(false);
    }
    ...
    if (value.length <= 1) {
      ...
      if (index === 5 && value) {
        ...
        if (isComplete) {
          ...
          void (async () => {
            const ok = await verifyCode(full);
            if (!ok) {
  ...
  return (
    <div className={cn("flex w-[100%] flex-col min-h-screen bg-black relative", className)}>
      ...
      <div className="relative z-10 flex flex-col flex-1">
        {/* Top navigation intentionally omitted (app navbar already renders) */}
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[1.2rem] text-white/70 font-light">
                        {mode === "sign-in" ? "Sign in to continue" : "Sign up to continue"}
                      </p>
                    </div>
                    ...
                    <div className="space-y-4">
                      <button
                        ...
                        className="backdrop-blur-[2px] w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white border border-white/10 rounded-full py-3 px-4 transition-colors disabled:opacity-60"
                      >
                        <span className="text-lg">G</span>
                        <span>{oauthSubmitting ? "Please wait..." : "Continue with Google"}</span>
                      </button>
                      ...
                      <form onSubmit={handleEmailSubmit} className="space-y-3">
                        ...
                        {error ? (
                          <div className="text-sm text-red-200 border border-red-500/30 bg-red-500/10 rounded-2xl px-4 py-2">
                            {error}
                          </div>
                        ) : null}

                        {mode === "sign-up" ? <div id="clerk-captcha" className="pt-2" /> : null}
                      </form>
                    </div>
                    ...
                    <div className="pt-6 text-sm text-white/60">
                      {mode === "sign-in" ? (
                        ...
                        <>
                          ...
                          <Link href="/sign-in" className="underline hover:text-white transition-colors">
                            Sign in
                          </Link>
                        </>
                      )}
